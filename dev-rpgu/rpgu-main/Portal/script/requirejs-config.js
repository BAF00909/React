requirejs.config({
	baseUrl: 'https://bitx-dev2.egspace.ru/bitrix/templates/rpgu-main/Portal',
...
var reconfigJq = function(jq) {
	jq.ajaxSetup({
		globalUrl: "https://testrpgu.egspace.ru",
	...
	jq.ajax = function(options) {
		...
		if (setup.globalUrl) {
			...
			if (options.headers == null || options.headers.proxy !== true) {
				...
				var setup = jq.ajaxSetup({
					url: 'https://bitx-dev2.egspace.ru/proxy.php' + '?_=' + Date.now(),
					crossDomain: true,
					headers: {
						'X-Proxy-URL': 'https://testrpgu.egspace.ru' + options.url,
