requirejs.config({
	baseUrl: 'http://bitx-dev2.egspace.ru/bitrix/templates/rpgu-main/Portal',
	...
	shim: {
		...
		'Nvx.ReDoc.WebInterfaceModule/Content/Scripts/Helpers/asyncSignLibDynamicLoad': {
			exports : 'Nvx.ReDoc.WebInterfaceModule/Content/Scripts/Helpers/asyncSignLibDynamicLoad'
		},
...
var reconfigJq = function(jq) {
	jq.ajaxSetup({
		...
		//globalUrl: "http://10.10.10.44:29929",
		cache: false
	});
	...
	jq.ajax = function(options) {
		...
		if (setup.globalUrl) {
			...
			if (options.headers == null || options.headers.proxy !== true) {
				...
				var setup = jq.ajaxSetup({
					url: 'http://bitx-dev2.egspace.ru/proxy.php' + '?_=' + Date.now(),
					crossDomain: true,
					headers: {
