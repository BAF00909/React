...
?><br xmlns="http://www.w3.org/1999/html"/><div class="search-page">
<form action="" method="get" class="search-area static search-page-form">
	<div class="field-holder">
		<input type="hidden" name="tags" class="form-control" placeholder="Поиск" value="<?echo $arResult["REQUEST"]["TAGS"]?>" />
	</div>
...
<?else:?>
	<div class="field-holder">
		...
		<button type="submit" class="btn"><i class="icon-zoom_white_desk"></i> Поиск</button>
	</div>
<?endif;?>
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
		<a class="service-plain-text-link" href="<?echo $arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a>
		<p class="plain-text--bold" style="margin-bottom: 0px; margin-top: 10px;"><?echo $arItem["BODY_FORMATED"]?></p>
		<?if (
		...
		if($arItem["CHAIN_PATH"]):?>
			<small class="result-small"><?=$arItem["CHAIN_PATH"]?></small><?
		endif;
		?>
		<small class="result-modified result-small"><?=GetMessage("SEARCH_MODIFIED")?> <?=$arItem["DATE_CHANGE"]?></small>
	<?endforeach;?>
	<?if($arParams["DISPLAY_BOTTOM_PAGER"] != "N") echo '<p class="result-nav-string">'.$arResult["NAV_STRING"].'</p>' ?>
	<p class="result-nav-string">
