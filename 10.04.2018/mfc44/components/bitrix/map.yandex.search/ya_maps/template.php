...
<div class="bx-yandex-search-layout">
	<div class="bx-yandex-search-form">
		<form name="search_form_<?echo $arParams['MAP_ID']?>" onsubmit="jsYandexSearch_<?echo $arParams['MAP_ID']?>.searchByAddress(this.address.value); return false;">
			<?echo GetMessage('MYMS_TPL_SEARCH')?>: <input type="text" id="map_text" name="address" value="" style="width: 300px;" /><input type="submit" id="map_button" value="<?echo GetMessage('MYMS_TPL_SUBMIT')?>" />
		</form>
	</div>

	<div class="bx-yandex-search-results map_href" id="results_<?echo $arParams['MAP_ID']?>"></div>
